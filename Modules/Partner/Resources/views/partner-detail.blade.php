...
<div class="col-md-12 col-sm-12 ">
  ...
  <h2><a href="{{ route('partners') }}" class="fa fa-angle-double-left" >&nbsp;&nbsp;Company Partners </a></h2>
  @if(Auth::user()->hasPermission('edit-partner'))
...
<div class="col-md-12">
  ...
  <div class="col-md-12">
    <div class="row">
      <div class="col-md-4">
        @if($partner->supporting_doc == "")
      <div > No Supporting Document</div>
      @else

      <a href="{{asset('moa/'.$partner->supporting_doc)}}" target="_blank;"> 
      <?php  $imageExtensions = ['jpg', 'jpeg','png'];
        ...
        if(in_array($extension, $imageExtensions)){  ?>
            <img src="{{asset('moa/'.$partner->supporting_doc)}}" style="height:220px;width:220px;border: 1px solid gray;">
        <?php }else { ?>
      ...
      <div class="col-md-8" style="padding: 20px;"><br>
        <h5><center>{{$partner->company_name}}<br>{{$partner->nature_partnership}}</center></h5>
         <div class=" col-md-12">
        <label  class="col-sm-3 col-form-label">Scope:</label>
        <label  class="col-sm-7 col-form-label">{{$partner->scope}}</label>
       </div>
       <div class=" col-md-12">
        ...
        <label  class="col-sm-7 col-form-label">{{$partner->status}}</label>
       </div>
      </div>
    </div>
    <br>
    <div class="row">
       <div class="row col-md-12">
        ...
        <div  class="col-sm-8 col-form-label">{{$partner->classification}}<br>
          <?php
            $numOfCols = 3;
            $rowCount = 0;
         ...
         @if($partner->classification == "School")
                <div class="row">
                @foreach($partnerCS as $vpcs)
                <div class="col-md-<?php echo $bootstrapColWidth; ?>">
                      <ul>
                        <li>{{$vpcs->school_code}}</li>
                      </ul>
                </div>
                  <?php $rowCount++; ?>
...
<form id="formValidate" action="{{route('updatePartner')}}" method="post" enctype="multipart/form-data" autocomplete="off" >
                                            {{ csrf_field() }}
  <input type="text"  name="partnerID" value="{{$partner->id}}" hidden>
    <div class="col-md-12">
      <div class="row">
      <div class="col-md-5" >
        @if($partner->supporting_doc == "")
      ...
      @endif
        <div class="form-group">
              ...
              <input type="text"  id="award_cert" name="award_cert" class="form-control" value="{{$partner->supporting_doc}}" hidden>
        ...
        <span class="text-danger">* Required Fields</span><br>
           <div class="row form-group">
                  ...
                  <input type="text" class="form-control" name="partner" value="{{$partner->company_name}}" required>
            </div>
          <div class="row col-md-12">
              <div  class="col-md-3">
          ...
          <div class="row col-md-12">
                  ...
                  <div class="col-md-6  col-sm-6">
                      ...
                      <input type="date" class="form-control" style="font-size:13px;" name="from" id="fromVal"value="{{$partner->from}}" required>
                  </div>
                  <div class="col-md-6 col-sm-6">
                      <label><span class="text-danger"></span>To</label>
                      <input type="date" class="form-control" name="to" style="font-size:13px;" id="toVal" value="{{$partner->to}}" >
      ...
      <div class="row">
        ...
        <fieldset id="schoolset" style='display:none;'>
          @endif
              ...
              <div class="col-md-6 col-sm-6">
                ...
                <div class="col-md-6 col-sm-6">
                    ...
                    @foreach($partnerCS as $epcs)
                      <option value="{{$epcs->id}}" selected>{{$epcs->school_code}}</option>
                  @endforeach
                    </select>
                    <br>
          ...
          @if($partner->classification == 'Program')
                ...
                <div class="row">
                    <div class="col-md-6 col-sm-6">
                        ...
                        @foreach($program as $prog)
                            <option value="{{$prog->id}}">{{$prog->acad_prog_code}}</option>
                        @endforeach
                        </select>
                        <br>
                    ...
                    <div class="col-md-6 col-sm-6">
                        ...
                        <select id="lbpselect" name="programc[]" multiple="multiple" class="form-control small" style="height: 150px;">
                          @foreach($partnerCP as $epcp)
                          <option value="{{$epcp->id}}" selected>{{$epcp->acad_prog_code}}</option>
                        @endforeach
                        </select>
                        <br>
      ...
      <?php
       ...
       $x = 0; 
      $count = count($lists);?>
      @foreach($natureList as $n)
          @if(!in_array($n,$array))
          <input type="text" class="form-control" name="nature[]" value="{{$n}}">
...
<div id="forRenew">
  <form action="{{route('renewPartner')}}" method="post" enctype="multipart/form-data" autocomplete="off" >
                                            {{ csrf_field() }}
  <input type="text"  name="renew_partnerID" value="{{$partner->id}}" hidden>
    <div class="col-md-12">
      <div class="row col-md-12">
        <h5>{{$partner->company_name}}<br>{{$partner->nature_partnership}}</h5>
      </div>
       <hr>
      <div class="row">
        <div class="col-md-4">
