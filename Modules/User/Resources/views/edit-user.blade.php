...
<div class="col-md-12 col-sm-12 ">
  ...
  @endif
  <form  action="{{route('updateUser')}}" method="POST" id="formValidate" enctype="multipart/form-data"  >
  {{ csrf_field() }}
            ...
            <div class="col-md-4">
		        	<label>Department</label>
		        	 <select id="mydropbox" name="school_id"class="form-control" onchange="copyValue()" required>
			            @foreach($school as $sc)
			            <option value="{{$sc->id}}" <?=$user->school_id == $sc->id ? ' selected="selected"' : '';?>>{{$sc->school_name}}</option>
			            @endforeach
                </select>
            </div>
		        <input type="textbox" id="user_id" name="user_id" value="{{$user->user_id}}" hidden/>
		        </div>
		        <div class="row form-group">
		        	<div class="col-md-4">
		        	...
		        	<div class="col-md-4">
		        		...
		        		<input type="text" class="form-control" name="middle_i" value="{{$user->middle_initial}}">
		        	</div>
		        	<div class="col-md-4">
            ...
            <div class="form-group">
              ...
              <div><input type="radio"  value="{{$role->id}}" name="role" <?=$user->role_id == $role->id ? ' checked="checked"' : '';?> required >&nbsp;&nbsp;{{$role->role_name}}</div>
              @endforeach
            </div>
		        <div class="row form-group">
              ...
              <table class="table table-bordered">
                  <tr>
                    ...
                    @foreach($permissions as $p)
                      <td><input type="checkbox"  value="{{$p->id}}" name="permission[]" <?=in_array($p->id, $lists) ? ' checked="checked"' : '';?>></td>
                      <?php $rowCount++; ?>
