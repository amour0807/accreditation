...
class UserAccreditationController extends Controller
{
    public function index()
    {
        ...
        return view('accreditation::schools.dashboard', compact('count1', 'count2', 'count3' ,'count4', 'count5', 'count6', 'expiring', 'school'));
        }else{
            return view('accreditation::schools.firstlogin', compact('user'));
        }
        $user = User::where('id', auth()->user()->id)->first();
    ...
    public function acred_prog()
    {
         $school = School::where('id', auth()->user()->school_id)->first();
         return view('accreditation::schools.index',compact('school'));
        
    }
     public function userAcademic_programs(){
        $school = School::where('id', auth()->user()->school_id)->first();
        return view('accreditation::schools.schoolAcadprog-details', compact('school'));
    }

 public function userAcadprog_dtb($id){
    $programs = School::join('acad_prgrms', 'acad_prgrms.school_id', 'schools.id')
        ...
        ->get();  
         ...
         return DataTables::of($programs)
            ...
            ->addColumn('actions', function($programs) {
                    return '
                        <button class="btn btn-secondary btn-sm edit" title="Edit" programid="'.$programs->id.'"><i class="far fa-edit"></i>
                        </button>
                        <button class="btn btn-danger btn-sm destroy" title="Remove" programid="'.$programs->id.'"><i class="far fa-trash-alt"></i>
                        </button>
                        ';
            })
        ...
        //Academic Program
     public function useraddAcadProg(Request $request){
    ...
    public function school_dtb(){
        ...
        ->addColumn('actions', function($school) {
                return '<a class="btn bg-ub-red btn-sm" href="'.route("userAccredited_programs" , $school->id).'">
         ...
         return DataTables::of($programs)
            ...
            ->addColumn('visit_date', function($programs) {
                if($programs->visit_date_to){
                     ...
                     $to = date('M. d, Y', strtotime($programs->visit_date_to));
                    return $from.' - '.$to;
                }else{
                    return $programs->visit_date_from;
                }
            ...
            ->addColumn('actions', function($programs) {
                    return '<a class="btn bg-ub-red btn-sm" href="'.route("userAccredDetails", $programs->a_id).'">
                            <i class="fa fa-eye" aria-hidden="true"></i>
                        </a>
                        
                        <a class="btn btn-secondary btn-sm" href="'.route("userAccredHistory", $programs->AcadPrgrm->id).'"><i class="fa fa-history" aria-hidden="true"></i>
                        </a>
                        ';
            })
    ...
    public function accredDetails($id){
        ...
        return view('accreditation::schools.accreditation-details', compact('program'));
    ...
    public function accredEdit($id){
        ...
        return view('accreditation::schools.accreditation-edit', compact('program', 'accredStats'));
    }
    public function saveEdit(Request $request)
    {
        ...
        $program = PrgrmAccred::where('id', $request->id)->first();
         ...
         return redirect()->route('userAccredDetails', $request->id)->with('success', 'Record Updated');
       
    }
    public function accredHistory($id){
        $program = PrgrmAccred::where('acad_prgrm_id', $id)->first();

        return view('accreditation::schools.history', compact('program'));
    ...
    public function add_accred_form(){
        $school = School::where('id', auth()->user()->school_id)->first();
        $programs = AcadPrgrm::where('school_id', $school->id)->get();
        $accredStats = AccredStat::all();

        return view('accreditation::schools.add-accreditation', compact('school', 'accredStats', 'programs'));
    ...
    public function accredited_programs($id){
        ...
        return view('accreditation::schools.accredited-programs', compact('school'));
    }
    ...
    public function accred_status(){
        return view('accreditation::schools.accred-status');
...
//reports
    ...
    public function accredReport(){
        ...
        return view('accreditation::schools.accreditation-report', compact('accreditations', 'schools', 'accredStats', 'programs'));
    ...
    public function viewProgramHistory(){
        ...
        return view('accreditation::schools.history-reports', compact('accreditations', 'schools', 'accredStats', 'programs'));
...
//Program Datatables
    ...
    public function filterReport(Request $request)
    {
        ...
        $queryBuilder = DB::table('prgrm_accreds')
            ...
            if($school){
                $queryBuilder =$queryBuilder->where('schools.id', $school->id);
            }
            if($program){
                $queryBuilder =$queryBuilder->where('acad_prgrms.acad_prog_code', $program);
            }
            if($accredStatus){
        ...
        $pdf = PDF::loadView('accreditation::schools.reports.accreditation-report', compact('queryBuilder', 'accredStatus', 'expiry', 'min', 'max', 'school', 'visitYear'));
    ...
    public function changePassword(Request $request){
        ...
        return redirect()->route('userAccredIndex')->with("success","Password changed successfully !");
    }
}
